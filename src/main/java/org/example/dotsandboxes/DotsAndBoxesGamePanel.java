...
public class DotsAndBoxesGamePanel extends JPanel {
    ...
    // Конструктор, который принимает gRPC клиента
    public DotsAndBoxesGamePanel(GameServiceGrpc.GameServiceBlockingStub blockingStub) {
        this.blockingStub = blockingStub;
        this.asyncStub = GameServiceGrpc.newStub(ManagedChannelBuilder.forAddress("localhost", 50051).usePlaintext().build());
    ...
    private void makeMove(Point p1, Point p2) {
        ...
        if (!squareMade) {
            togglePlayer();
            // Here you'd send a gRPC message to the server for the move
            makeMoveRequest(p1, p2);
        }

        firstPoint = null;
    }

    private void makeMoveRequest(Point p1, Point p2) {
